...
test.describe("HITO 4 — accesibilidad", () => {
  ...
  test("AC01 — movimiento reducido expone contenido e interacciones", async ({ page }) => {
  ...
  test("AC02 — conserva el texto narrativo semántico", async ({ page }) => {
    await reachPresent(page, 0.1);
    const semanticText = await page.locator(".present__writing-semantic").innerText();
  ...
  test("AC03 — movimiento reducido mantiene la señal en reposo", async ({ page }) => {
    await page.emulateMedia({ reducedMotion: "reduce" });
    await reachPresent(page, 0.1);
